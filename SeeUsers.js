...
const API_URL = 'http://192.168.255.85:80/apis/get_users.php';
...
class SeeUsers extends Component {
  ...
  handleDelete = async (id) => {
    const response = await fetch(`http://192.168.255.85:80/apis/delete_user.php?id=${id}`);
    const json = await response.json();
  ...
  handleSave = async (id, data) => {
    const response = await fetch(`http://192.168.255.85:80/apis/update_user.php?id=${id}`, {
      method: 'POST',
